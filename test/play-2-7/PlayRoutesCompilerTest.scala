// Play 2.7's `WithApplication` runs the example body directly instead of through a
// `running()` override, so this flavor keeps its own copy of the specs.
...
class PlayRoutesCompilerTest extends Specification {
  ...
  "Play Routes Compiler when run through Bazel" should {
    "Compile Play routes" in new WithApplication(application) {
    ...
    "Handle Large Routes files well" in new WithApplication(application) {
      status(route(app, FakeRequest(GET, "/large/749")).get) mustEqual OK
    }

    "Support multiple routes files" in new WithApplication(application) {
      status(route(app, FakeRequest(GET, "/ok2")).get) mustEqual OK
      status(route(app, FakeRequest(GET, "/ok3")).get) mustEqual OK
    }

    "Add additional imports when specified" in new WithApplication(application) {
      status(route(app, FakeRequest(GET, "/okUser/1")).get) mustEqual OK
    }

    "Use the specified routes generator" in new WithApplication(application) {
      status(route(app, FakeRequest(GET, "/okGenerator")).get) mustEqual OK
    }

    "Generate reverse routes when specified" in new WithApplication(application) {
      val myRoute = ReverseRoutesController.ok(3).toString
      status(route(app, FakeRequest(GET, myRoute)).get) mustEqual OK
    }
  }
}
